    ...
    if state.current_revision_idea_id:
        ...
        if not ideas:
            raise ValidationError(
                f"Idea {state.current_revision_idea_id} not found for revision scoring"
            )
        return ideas
    else:
...
def _serialize_pain_points(pain_points: list) -> list[dict]:
    ...
    """
    sorted_pps = sorted(pain_points, key=lambda pp: len(pp.evidence), reverse=True)
    return [
        {
            "id": str(pp.id),
            "title": pp.title,
...
def _calculate_yes_count(
    f_rubric: FeasibilityRubric, d_rubric: DemandRubric, n_rubric: NoveltyRubric
) -> int:
...
def _merge_revision_scores(
    state: VentureForgeState, new_scores: list[ScoredIdea]
) -> list[ScoredIdea]:
...
def _build_success_patch(
    state: VentureForgeState, all_scores: list[ScoredIdea], new_scores_count: int
) -> dict:
