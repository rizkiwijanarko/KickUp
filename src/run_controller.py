...
Gradio UI and API consumers can:

* start a new run (returns run_id)
* poll the latest state for a run_id via SQLite persistence
...
def stream_run_events(
    ...
    *,
    ...
    """Yield (node_name, chunk) tuples in real-time as the graph executes."""
    run_id = state.run_id
    config = {
    ...
    for chunk in GRAPH.stream(state, config=config):
        if is_cancel_requested(run_id):
            logger.info(f"[run_controller] Cancellation requested for {run_id}, stopping stream")
            break
    ...
    *,
    ...
    def _worker() -> None:
        ...
        try:
            ...
            for node_name, node_output in stream_run_events(state):
                step_count += 1
            ...
            # If cancelled, write cancellation state
            if is_cancel_requested(run_id):
