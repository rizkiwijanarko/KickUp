...
def test_wellformed_response_validates_ideas() -> None:
    ...
    with patch("src.agents.idea_generator.get_llm") as mock_get_llm:
        ...
        # Return a single idea (one-idea-at-a-time generation)
        fake_response.content = json.dumps(good)
    ...
    assert len(ideas) == 2, f"Expected 2 validated ideas (ideas_per_run=2), got {len(ideas)}"
...
def test_unwrapped_list_response_supported() -> None:
    state = _make_minimal_state()
    pp_ids = [pp.id for pp in state.pain_points]
    single_idea = _make_idea_dict(title="Unwrapped", pp_ids=[str(pp_ids[0]), str(pp_ids[1])])

    with patch("src.agents.idea_generator.get_llm") as mock_get_llm:
        fake_llm = MagicMock()
        fake_response = MagicMock()
        # Return a single idea (one-idea-at-a-time generation)
        fake_response.content = json.dumps(single_idea)
        fake_llm.invoke.return_value = fake_response
        mock_get_llm.return_value = fake_llm
