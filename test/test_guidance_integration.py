    ...
    try:
        ...
        if "134.199.205.81" in settings.llm_base_url:
            ...
            try:
                ...
                print(f"     Note: vLLM has limited Guidance support (no SelectNode, no stop conditions)")
                print(f"     Fallback to standard LLM recommended for production use.")
                
                # Verify it's a reasonable answer (may be empty with vLLM)
                if answer and ("4" in answer or "four" in answer.lower()):
                    print(f"     [OK] Answer contains expected value")
                else:
