...
def test_1_multi_source_pain_point_schema():
    ...
    pp = PainPoint(
        ...
        rubric=PainPointRubric(
            is_genuine_current_frustration=True, has_verbatim_quote=True, user_segment_specific=True
        ),
        passes_rubric=True,
        evidence=evidence,
    )
    ...
    assert pp.source_url == "https://news.ycombinator.com/item?id=1", (
        "Primary source_url should be first evidence"
    )
    assert pp.raw_quote == "Debugging microservices is a nightmare", (
        "Primary raw_quote should be first evidence"
    )
...
def test_2_critic_rubric_has_8_checks():
    ...
    assert not hasattr(rubric, "unscalable_acquisition_concrete"), (
        "Manual outreach check should be removed"
    )
    assert not hasattr(rubric, "gtm_leads_with_manual_recruitment"), (
        "GTM manual recruitment check should be removed"
    )
    ...
    print(
        "  ✓ New checks: minimum_evidence_sources, scorer_verdict_justified, validation_plan_complete"
    )
    return True
...
def test_5_evidence_validation():
    ...
    evidence_list = [
        PainPointEvidence(
            source_url=f"https://example.com/{i}", raw_quote=f"Quote {i}", source=DataSource.WEB
        )
